...
const fieldEquals = (field, value) => products.filter(product => {
...
const sortByPrice = (direction) => (direction == ASCENDING)
  ? products.sort((a, b) => b.price - a.price)
  : products.sort((a, b) => a.price - b.price);


const isOutOfStock = product => !product.stock
const findOutOfStock = _ => products.filter(isOutOfStock); // handle undefined, null, string

function getCategoryStockValue(category) {
  const sum = findByCategory(category)
 ...
 * @returns
 */
const findProductsGreaterThanPrice = (price) => products.filter(product => product.price > price)
...
const totalCountsByCategory = () => {
  ...
  products.forEach(product => {
    if (!totals.hasOwnProperty(product.category)) // there is a data structure for this.. i forget what its called
